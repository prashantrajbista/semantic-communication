...
def apply_channel(sym: np.ndarray, kind: str, snr_db: float, rician_k: float = 1.0,
    ...
    DeepSC-S gets at the paper's dimensions (K = L*depth/2 = 128*8/2).
...
def assert_matched_rho(n_samples: int = 16384, block_len: int = 512, depth: int = 8):
    ...
    rho_b, rho_n = bandwidth_ratio(n_samples, block_len), neural_rho(depth)
